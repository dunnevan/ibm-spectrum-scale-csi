...
# image if the QUAY_BOT_USERNAME and QUAY_BOT_PASSWORD environment variables
# are set.
#
function docker_login() {
  image_name=$1; shift || fatal "${FUNCNAME} usage error"

  if [[ -n "$QUAY_BOT_USERNAME" && -n "$QUAY_BOT_PASSWORD" ]]; then
    server=$(registry_of $image_name)
    echo "$QUAY_BOT_PASSWORD" | docker login -u "$QUAY_BOT_USERNAME" --password-stdin "$server"
  fi
}
...
#
# registry_of <image_name>
#
# registry_of returns the server component of the image
